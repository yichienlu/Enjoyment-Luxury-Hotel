...
function FoodList(){
  ...
  React.useEffect(()=>{
    if(food_list.current){
      food_list.current.addEventListener('pointerdown', ()=>changeMovable(true))
      food_list.current.addEventListener('pointerup', ()=>moveFoodList)
    }
  })
  const changeMovable = (condition:boolean):void => {
    // console.log('Element clicked!');
    if(food_list.current){
      setMovable(condition)
      food_list.current.style.cursor = 'grabbing' 
      console.log(condition,movable)
    }
  };
  const moveFoodList = ():void => {
    // if(movable){
      console.log(movable)
    // }
  }
